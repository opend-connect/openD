...
{
  openD_hanfunApi_devMgmtInd_t hDevMgmtInd;

  EXPECT_EQ( OPEND_STATUS_FAIL, openD_hanfun_devMgmtInd(&hDevMgmtInd) );
}
 ...
 *          an initialization.
 */
TEST_F( ut_opend_hanfun_devmgmtind, ut_opend_hanfun_devmgmtind_valid_Pointer )
{
  /* Set the confirmation and indication callbacks. */
  openD_hanfunApiPrimitives.openD_hanfunApi_mgmtInd = devMgmtIndCallback;

  EXPECT_EQ( openD_hanfunApi_init(&openD_hanfunApiPrimitives), OPEND_STATUS_OK );

  openD_hanfunApi_devMgmtInd_t hDevMgmtInd;

  EXPECT_EQ( OPEND_STATUS_OK, openD_hanfun_devMgmtInd(&hDevMgmtInd) );
}
