...
struct swTimer {
  struct swTimer *next;
  uint32_t timeout;
  openD_sw_timeoutCallback timeoutCallback;
};
...
void openD_sw_timer_tick( uint16_t elapsedTime ) {
  ...
  while( ptr != NULL ) {
    if( currentTime >= ptr->timeout ) {
      openD_sw_timeoutCallback callback;
...
void openD_sw_timer_start( uint16_t timeout, openD_sw_timeoutCallback timeoutCallback ) {
  ...
  if( ( first == NULL ) ||
    ...
    first = (swTimer_t *) malloc( sizeof( swTimer_t ) );
    ...
    while( ptr->next != NULL ) {
      ...
      if( ( ptr->timeout - currentTime ) > timeout ) {
         ...
         *    Insert it here.
         */
        ptrLastOne->next = (swTimer_t *) malloc( sizeof( *ptr ) );
     ...
     */
    ptr->next = (swTimer_t *) malloc( sizeof( *ptr ) );
